...
cd ..
# Get the results
paste ${language}.tst.seq.sc.base sequitur/rerank.tst.out | cut -f 1-3 | rerank.py | \
    tee ${language}.tst.seq.sc.reranked | eval.py ${language}.tst; mcNemars.py ${language}.tst \
    ${language}.tst.seq.sc.{base,reranked}
#Word accuracy: 50.05%
...
echo "NEWS results for seq reranked"
sed 's/ //g' ${language}.tst.seq.sc.reranked > tmp
sed -e 's/#/_/g' -e 's/%/ /g' tmp >  ${language}.seq.sc.final # convert back to spaces and underscoress
evalDirecTL.sh ${language}.seq.sc.final ../../dev.xml