...
# get SMT output now
cat ../${UPPER}.9.tst.smt.out | sed -e 's/_/#/g' -e 's/ /%/g' > ${language}.trn.smt
cat ../${UPPER}.dev.smt.out | sed -e 's/_/#/g' -e 's/ /%/g' > ${language}.tst.smt
...
cut -f 1,2 ${language}.trn.dtlp | sed -e 's/[|_]//g' -e '/^$/d' | getTop1.py > ${language}.trn.dtlp.top1
cut -f 1,2 ${language}.tst.dtlp | sed -e 's/[|_]//g' -e '/^$/d' | getTop1.py > ${language}.tst.dtlp.top1

cut -f 2,3 ${language}.trn.smt | sed -e '/^$/d' | getTop1.py > ${language}.trn.smt.top1
cut -f 2,3 ${language}.tst.smt | sed -e '/^$/d' | getTop1.py > ${language}.tst.smt.top1

mkdir suppl; cd suppl
declare -A sups
sups=([dtlp]=dt [seq]=sq [smt]=st)
for sup in ${!sups[@]}
do
